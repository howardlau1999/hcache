...
- 计分公式：$\mathrm{score} = \max\\{\frac{10}{T}, 10\\}+10n+(\frac{50k\cdot\mathrm{qps}}{10000})+\frac{50m}{\mathrm{time\\_ delay}}$，其中，$T$ 为加载完成的时间（只在第一次启动的时候计时，后续重启不计），$n$ 为评测的接口个数（正确性），$k$ 是需要进行评测 QPS 的接口个数（接口性能），$m$ 是需要评测延迟的接口个数，$time\_delay$ 是接口的平均延迟。
