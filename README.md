...
具体工程细节上，计划使用 Thread Per Core 和 ~~io-uring~~（实际表现有点拉胯） 来实现，使用异步编程的方式提高并发度。更进一步，将不同的 Key 分区到不同的 Core 上去执行。当然也可以全局共享一个数据结构，但这样可能会导致激烈的竞争。另外，还可以使用 DPDK（大请求 QPS 直接慢两个数量级，原因不明）和 SPDK 来将网络和 IO 操作移到用户态去做。
...
- **2022.07.08(howard)**: 用 tcpdump 抓包分析了一下为什么 batch 延迟这么高，发现有很多的 Dup ACK 和 Retransmission，说明丢包问题很严重，暂时不知道是 TCP 实现有问题还是阿里云的问题。大概率是 Seastar 的 TCP 接收算法有问题，因为 list 或者 zrange 这种就没什么问题。
- **2022.07.07(howard)**: Fedora QPS 能稳定 14000+，但是 delay_score 上不去 3000，可能是评测系统玄学。添加了 Seastar 的 Sharded 版本，每个 CPU 维护一个单线程的数据结构，然后通过 `distributed<>` 的 `invoke_on` 根据 key 的哈希去调用。
- **2022.07.06(howard)**: DPDK 还是没有跑通，分数还是很低，score:1928.2548, init_score:3.3333, api_score:630.0000, qps_score:1294.8714, delay_score:0.0500。而且普通的 Seastar 也跑不通了，开了 `poll-mode` 也不行，非常玄学。重新交了一次 Drogon，score:16494.7257, init_score:10.0000, api_score:630.0000, qps_score:13574.4227, delay_score:2280.3030。改了一下[内核参数和一些网卡调优](https://talawah.io/blog/linux-kernel-vs-dpdk-http-performance-showdown/)，作者还在另外一篇博客提到怎么[优化 HTTP 服务器](https://talawah.io/blog/extreme-http-performance-tuning-one-point-two-million/)以及[相关的 AWS CloudFormation 脚本](https://gist.github.com/talawahtech/ce2fe1f6a3e3851d15e912e0a4e93734)。用改后的内核参数 Rust 跑 score:17225.0448, init_score:5.0000, api_score:630.0000, qps_score:14081.7115, delay_score:2508.3333。QPS 应该是跑出来最高的了，说明优化还是有一点用的，但是为什么 delay_score 拉胯了。换了 Fedora 35，内核版本 5.18，用 Rust 跑 score:17659.22, init_score:10.00, api_score:630.00, qps_score:13817.09, delay_score:3202.13
...
- **2022.06.25(howard)**: 写了镜像相关的脚本，交了第一版上去，600.05 分，QPS 分为 0，API 分没有拿满（满分应该是 900）。写了客户端 Benchmark，自定义服务器 main。添加逻辑，对普通 KV 进行 zadd 操作的时候报错。
