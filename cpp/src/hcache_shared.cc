...
public:
  virtual future<std::unique_ptr<seastar::httpd::reply>> handle(
    ...
    auto const &arr = parser.parse(json);
    for (auto &&key: arr) { keys.insert(key.get_string().take_value()); }
...
public:
  virtual future<std::unique_ptr<seastar::httpd::reply>> handle(
    ...
    auto &&key = folly::fbstring(key_value.data(), slash_ptr - key_value.data() - 1);
    auto &&value = folly::fbstring(slash_ptr, end_ptr - slash_ptr);
